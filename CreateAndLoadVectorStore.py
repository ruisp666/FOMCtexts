...
if __name__ == '__main__':
    folder_docs = 'test_dcos_folder/'
    timestamps = [f[-12:-6] for f in glob.glob(f"{folder_docs}/*.pdf")]
    timestamps_docs = {t: [f for f in glob.glob(f"{folder_docs}/*{t}*.pdf")] for t in timestamps}
    minutes_mall = VectorMall(docs_folder=folder_docs,grouped_docs=timestamps_docs)
    minutes_mall.split_into_stores()
